...
export default function Programmers() {
  ...
  const [programList, programListSet] = useState(listOfAwesome);
  const [programId, programIdSet] = useState(0);
  ...
  const style = {
    fontSize: '1.5em',
    marginTop: '0.5em',
    color:  (programId != 0 ? 'gold' : 'royalblue')
  };
      ...
      <div className='programmers'>
        {
          ...
          listOfAwesome.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { programIdSet(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
